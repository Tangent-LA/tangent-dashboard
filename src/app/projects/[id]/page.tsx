...
import type { Project, Team, ActivityLog, ProjectStage, ProjectPriority, ProjectStatus } from '@/types';
  ...
  const fetchProjectDetails = async () => {
    ...
    try {
      ...
      const { data: activitiesData } = await supabase
        ...
        .limit(10);
      if (activitiesData) setActivities(activitiesData as ActivityLog[]);
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <h2 className="text-lg font-semibold mb-6 flex items-center gap-2 text-tangent-blue">
                📋 Project Information
              </h2>
              ...
              {isEditing ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditForm({ ...editForm, criticality: parseInt(e.target.value) })}
                        className="input-field"
                      />
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-tangent-blue/5 rounded-xl border border-dark-border">
                  ...
                  <div className="p-4 bg-tangent-blue/5 rounded-xl border border-dark-border">
                    ...
                    <span className={cn('badge border', statusStyle.bg, statusStyle.text, statusStyle.border)}>
                      {project.status}
                    </span>
                  </div>
                  <div className="p-4 bg-tangent-blue/5 rounded-xl border border-dark-border">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Priority</p>
                    <span className={cn('badge border', priorityStyle.bg, priorityStyle.text, priorityStyle.border)}>
                  ...
                  <div className="p-4 bg-tangent-blue/5 rounded-xl border border-dark-border">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Criticality</p>
                    <p className="font-semibold">{project.criticality}/10</p>
                  </div>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              <div className="relative pl-6 border-l-2 border-dark-border space-y-6">
                {activities.length === 0 ? (
                  <p className="text-gray-500 text-sm">No activity recorded yet</p>
                ) : (
                  activities.map((activity, index) => (
                    <div key={activity.id} className="relative">
                      <div className="absolute -left-[25px] top-1 w-3 h-3 bg-tangent-blue rounded-full" />
                      <p className="text-xs text-gray-500 mb-1">{formatDateTime(activity.created_at)}</p>
                      <p className="text-sm">
                        Project <span className="text-tangent-blue font-medium">{activity.action}</span>
                      </p>
                    </div>
                  ))
                )}
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              className="glass-card p-6"
            >
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-tangent-blue">
                <Timer className="w-5 h-5" />
                Deadline Countdown
              </h2>
              <div className="text-center py-4">
                <p className={cn(
                  'text-6xl font-bold bg-gradient-tangent bg-clip-text text-transparent'
                )}>
                  {Math.abs(daysRemaining)}
                </p>
                <p className={cn('text-lg mt-2', deadlineStyle.text)}>
                  {deadlineStatus === 'overdue' ? 'Days Overdue' : 'Days Remaining'}
                </p>
              </div>
            </motion.div>
